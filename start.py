...
class UiLoadingWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        super(UiLoadingWindow, self).__init__(parent)
        ...
        self.frame.setStyleSheet("\n"
                                 ...
                                 "32, 178, 170, 255), stop:1 rgba(144, 238, 144, 255));    \n "
                                 "    border-radius: 15px;\n"
                                 "}")
        ...
        self.ProgressBar.setStyleSheet("QProgressBar\n"
                                       ...
                                       "rgba(227, 227, 227, 255), stop:1 rgba(156, 156, 156, 255));\n "
                                       "    border-style:none;\n"
                                       "    border-radius:20px;\n"
                                       "}\n"
                                       "QProgressBar::chunk\n"
                                       ...
                                       "    border-radius:20px;\n"
                                       "background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 "
                                       "rgba(61, 255, 249, 255), stop:1 rgba(38, 197, 255, 255));\n "
                                       "}")
    ...
    def translate_ui(self, loading_winodw):
        _translate = QCoreApplication.translate
        loading_winodw.setWindowTitle(_translate("LoadingWinodw", "Form"))
    ...
    def set_loader(self):
        ...
        self.timer.timeout.connect(self.change_loading_label)
        self.timer.start(15)

    def load_progress_bar(self):
        UiLoadingWindow.ProgressBarValue += 1
        self.ProgressBar.setValue(UiLoadingWindow.ProgressBarValue)
        if UiLoadingWindow.ProgressBarValue >= 150:
            self.timer.stop()
            self.open = interface.UiForm()
            self.open.show()

    def change_loading_label(self):
        i = int(UiLoadingWindow.ProgressBarValue / 10)
        if i >= 10:
